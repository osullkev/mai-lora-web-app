...
for (var k in sentUplinks)
{
    ...
    // console.log(ul);
    console.log(ul.tx_response);
    var receivedIndex = ul.fw_seg_ack;
    console.log(ul.fw_seg_ack);
    if (ul.tx_response == "successful_with_response")
    {
        if (receivedFWSegments[receivedIndex])
        {
            console.log(receivedIndex + ": already received");
            ineffectiveDataUplink++;
        }
        else
        {
            effectiveUplink++;
            receivedFWSegments[receivedIndex] = true;
            console.log(receivedIndex + ": unique");
        }
    }
    else
    {
...
var ineffectiveUplink = ineffectiveResponseUplink + ineffectiveDataUplink;
testResultsLogger("Effective Uplinks: " + effectiveUplink);
testResultsLogger("Ineffective Uplinks: " + ineffectiveUplink);
testResultsLogger("|");
testResultsLogger("+--- Ineffective Data Uplinks: " + ineffectiveDataUplink);
